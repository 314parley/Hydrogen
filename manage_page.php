...
$action = isset($_REQUEST['action']) ? $_REQUEST['action'] : 'announcement';
switch ($action) {
    case 'logout':
        $manage_class->Logout();
        break;
...
function manage_page($action = 'announcement') {
    ...
    if (is_callable(array($manage_class, $action))) {
        ...
        $tpl_page .= sprintf(_gettext('%s not implemented.'), htmlentities($action));
    }
