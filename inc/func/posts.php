...
function embeddedMusicBox($post, $board) {
	if(KU_MP3 == 'google') {
		...
		$output = '<span style="float: left;">' . "\n";
		$output .= '<embed type="application/x-shockwave-flash" src="'.KU_WEBPATH.'/player.swf?music='. KU_WEBPATH . '/' . $board . '/src/'. $post['filename'] . '.' . $post['filetype'] . '" width="80" height="15" /></embed>';
		$output .= '</span>&nbsp;' . "\n";
	}
	return $output;
}
