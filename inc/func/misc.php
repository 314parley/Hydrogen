...
function management_addlogentry($entry, $category = 0, $forceusername = '') {
	...
	$results = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "modlog` WHERE `entry` = '" . mysql_real_escape_string($entry) . "'");
	if (count($results) > 0) {
		if(strstr($entry, 'Deleted post')) {		
			return;
		}
	}
	if ($entry != '') {
	...
	if (KU_RSS) {
		...
		$rss_class = new RSS();
		if(KU_MODLOG != '') {
			print_page(KU_BOARDSDIR . KU_MODLOG . '.xml', $rss_class->GenerateModLogRSS($entry), '');
		}
	}
}
