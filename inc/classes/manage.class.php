 ...
 * +------------------------------------------------------------------------------+
 */
$ad_head = KU_ROOTDIR . 'hydrogen/ad_top.html';
$ad_foot = KU_ROOTDIR . 'hydrogen/ad_base.html';
$content1 = htmlspecialchars(file_get_contents(KU_ROOTDIR . 'hydrogen/ad_top.html'));
$content2 = htmlspecialchars(file_get_contents(KU_ROOTDIR . 'hydrogen/ad_base.html'));
class Manage {
	...
	/* Show the form and halt execution */
	function LoginForm() {
		global $tc_db, $smarty, $tpl_page;
	...
	function headadmanagement() {
		global $tpl_page;
		$this->AdministratorsOnly();
		if (!empty($_POST['enter ad here'])) {
			$tpl_page .= '<hr />Ad Sucessfully edited.<hr />';
		}
		$content1 = htmlspecialchars(file_get_contents(KU_ROOTDIR . 'hydrogen/ad_top.html'));
		$tpl_page .= '<h2>Edit Header Ad</h2> <br />' . "\n" .
					'<form action="?action=adpost_head" . method="post">' . "\n" .
					'<textarea name="ad1" rows="25" cols="80">' . $content1 . '</textarea><br />' . "\n" .
					'<input type="submit" value="submit" />' . "\n" .
					'</form>' . "\n";
				
	}
	function baseadmanagement() {
		global $tpl_page;
		$this->AdministratorsOnly();
		if (!empty($_POST['enter ad here'])) {
			$tpl_page .= '<hr />Ad Sucessfully edited.<hr />';
		...
		$content2 = htmlspecialchars(file_get_contents(KU_ROOTDIR . 'hydrogen/ad_base.html'));		
		$tpl_page .= '<h2>Edit Footer Ad</h2> <br />' . "\n" .
					'<form action="?action=adpost_base" method="post">' . "\n" .
					'<textarea name="ad2" rows="25" cols="80">' . $content2 . '</textarea><br />' . "\n" .
					'<input type="submit" value="submit" />' . "\n" .
					'</form>' . "\n";
	...
	function boardanalytics() {
		global $tpl_page;
		$this->AdministratorsOnly();
		$board_a = htmlspecialchars(file_get_contents(KU_ROOTDIR . 'hydrogen/analytics_board.html'));
		$tpl_page .= '<h2>Edit Board Analytics Script</h2> <br />' . "\n" .
					'<form action="?action=analytics_board" . method="post">' . "\n" .
					'<textarea name="a_bd" rows="25" cols="80">' . $board_a . '</textarea><br />' . "\n" .
					'<input type="submit" value="submit" />' . "\n" .
					'</form>' . "\n";
	...
	function menuanalytics() {
		global $tpl_page;
		$this->AdministratorsOnly();
		$menu_a = htmlspecialchars(file_get_contents(KU_ROOTDIR . 'hydrogen/analytics_menu.html'));
		$tpl_page .= '<h2>Edit Menu Analytics Script</h2> <br />' . "\n" .
					'<form action="?action=analytics_menu" . method="post">' . "\n" .
					'<textarea name="a_mn" rows="25" cols="80">' . $menu_a . '</textarea><br />' . "\n" .
					'<input type="submit" value="submit" />' . "\n" .
					'</form>' . "\n";
				
	}
	function analytics_board()
	{
				file_put_contents(KU_ROOTDIR . 'hydrogen/analytics_board.html', $_POST['a_bd']);
				echo '<h2>Sucess!</h2>';
	}
	function analytics_menu()
	{
				file_put_contents(KU_ROOTDIR . 'hydrogen/analytics_menu.html', $_POST['a_mn']);
				echo '<h2>Sucess!</h2>';
	}
	function adpost_head()
	{
				file_put_contents(KU_ROOTDIR . 'hydrogen/ad_top.html', $_POST['ad1']);
				echo '<h2>Sucess!</h2>';
	}
	function adpost_base()
	{
				file_put_contents(KU_ROOTDIR . 'hydrogen/ad_base.html', $_POST['ad2']);
				echo '<h2>Sucess!</h2>';
	}
	...
	function bans() {
		...
		if ($isquickban) {	
			...
			<div class="desc">If checked, the post will be deleted as well.</div><br/><input type="submit" value="Go" name="quicksubmit"/></form></fieldset>';
		}
		$tpl_page .= '<fieldset>
		...
		$tpl_page .= '<label for="postbanmsg">'._gettext('Post Ban Message').':</label>
		<input type="text" name="postmsg"><div class="desc">instead of <b><font color="red">(USER WAS BANNED FOR THIS POST)</font></b> - Not functioning for this release. I will have it working for b10</div><br/>';
		if (KU_APPEAL != '') {
