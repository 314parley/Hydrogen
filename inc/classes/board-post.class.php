...
class Board {
	...
	function Board($board) {
		...
		if ($board!='') {
			...
			$this->board_postboxnotice = $this->FormatPostbox(KU_POSTBOX . '<br><center>' . HY_HEADERAD . '</center>', $this->board_dir);
	...
	*/
	...
	function BuildPost($page, $post_board, $post_board_type, $post, $thread_replies=0, $thread_relative_id='', $reply_relative_id=0, $threads_on_front_page=0) {
		...
		if ($post_board_type != 1) {
			...
			'		<label>' . "\n" . 
			'			<input type="checkbox" id="delete" name="delete" value="' . $post['id'] . '">' . "\n";
			if(KU_MULTIPLE) {
				$info_post .= '<input type="radio" id="delete_thread" name="delete_' . $post['id'] . '" value="' . $post['id'] . '"></span>' . "\n";
			}
			if ($post['subject']!='') {
	...
	function PageHeader($replythread = '0', $liststart = '0', $listpage = '-1', $liststooutput = '-1', $isoekaki = false, $hidewatchedthreads = false) {
		...
		if ($this->board_type == 0 || $this->board_type == 2 || $this->board_type == 3) {
			$output .= '<div class="adminbar"><select name="switcher">' . "\n";
			if (KU_STYLESWITCHER) {
			...
			$output .= '</select>&nbsp;[<a href="'.KU_WEBPATH.'" target="_top">' . _gettext('Home') . '</a>]&nbsp;[<a href="' . KU_CGIPATH . '/manage.php" target="_top">' . _gettext('Manage') . '</a>]</div>' . "\n" .
...
size="28" maxlength="10" accesskey="c">' . "\n" .
						'	</td>' . "\n" .
						'</tr>' . "\n";
					}
					if ($this->board_enablefaptcha == 1) {
						$output .= '<tr>' . "\n" .
						'	<td class="'.$label_class.'">' . "\n" .
						'		<a href="#" onclick="javascript:document.getElementById(\'faptchaimage\').src = \'' . KU_CGIPATH . '/faptcha.php?\' + Math.random();return false;"><img id="faptchaimage" src="' . KU_CGIPATH . '/faptcha.php" border="0" width="90" height="25" alt="Faptcha image"></a>' . "\n" .
						'	</td>' . "\n" .
						'	<td>' . "\n" .
						'		<input type="text" name="faptcha"
size="28" maxlength="10" accesskey="f"> [<a href="'.KU_CGIPATH .'/news.php?p=faq" target="_blank"/>?</a>]' . "\n" .
						'	</td>' . "\n" .
						'</tr>' . "\n";
					}
					$output .= '<tr>' . "\n" .
	...
	function Footer($noboardlist = false, $executiontime = '', $hide_extra = false) {
		...
		$footer = '- <a href="http://code.google.com/p/hydrogenboard/" target="_top">Hydrogen ' . KU_VERSION . '</a> ';
		if ($executiontime != '') {
		...
		$footer .= '<center>' . '<b>' . HY_DISCLAIMER . '</b><br>' . HY_FOOTERAD . '</center>';
