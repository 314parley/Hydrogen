...
class Bans {
	...
	function BanCheck($ip, $board = '', $force_display = false) {
		...
		$results = $tc_db->GetAll("SELECT * FROM `".KU_DBPREFIX."banlist` WHERE ( `ipmd5` = '" . md5($ip) . "' OR `ipmd5` = '". md5($_COOKIE['tc_previousip']) . "' ) LIMIT 1");
		if (count($results)>0) {
		...
		if (count($results)>0) {
			foreach($results AS $line) {
				$ip_decrypt = md5_decrypt($line['ip'], KU_RANDOMSEED);
				$ip_regex = str_replace('*', '([0-9]|[1-9][0-9]|1([0-9][0-9])|2([0-4][0-9]|5[0-5]))', $ip_decrypt);
				if (eregi($ip_regex, $ip)) {
					echo $this->DisplayBannedMessage($line['globalban'], '<b>/'.implode('/</b>, <b>/', explode('|', $line['boards'])).'/</b>&nbsp;', $line['reason'], $line['at'], $line['until'], $line['appealat']);
					die();
				}
			}
		...
		if ($force_display) {
			...
			if (count($results)>0) {
				foreach($results AS $line) {
					...
					echo '<title>'._gettext('Banned!').'</title><div align="center"><img src="'.KU_BOARDSPATH.'/youarenotbanned.jpg" alt="You are banned"/><br><br>'._gettext('You have been banned for: <b>'). $line['reason'] . '</b><br/><br/>If no reason is specified please contact the administrator, testhdd@gmail.com.</div>';
				}
			}
		} else {
			return true;
	...
	function BanUser($ip, $modname, $globalban, $duration, $boards, $reason, $appealat=0, $type=0, $allowread=1, $note) {
		global $tc_db;
		...
		if ($result[0]==0) {
			...
			$tc_db->Execute("INSERT INTO `".KU_DBPREFIX."banlist` ( `ip` , `ipmd5` , `type` , `allowread` , `globalban` , `boards` , `by` , `at` , `until` , `reason`, `appealat` , `note`) VALUES ( '".md5_encrypt($ip, KU_RANDOMSEED)."' , '".md5($ip)."' , '".$type."' , '".$allowread."' , '".$globalban."' , '".$boards."' , '".$modname."' , '".time()."' , '".$ban_until."' , '".$reason."' , '".$appealat."', '".$note."' )");
	...
	function UpdateHtaccess() {
		...
		$results = $tc_db->GetAll("SELECT `ip` FROM `".KU_DBPREFIX."banlist` WHERE `allowread` = 0 ORDER BY `ip` ASC");
		if (count($results) > 0) {
			$htaccess_contents_bans_iplist .= 'RewriteCond %{REMOTE_ADDR} (';
			foreach($results AS $line) {
				$ip = md5_decrypt($line['ip'], KU_RANDOMSEED);
				$ip_regex = str_replace('*', '([0-9]|[1-9][0-9]|1([0-9][0-9])|2([0-4][0-9]|5[0-5]))', $ip);
				$htaccess_contents_bans_iplist .= str_replace('.', '\\.', $ip_regex) . '|';
			}
		...
		if ($htaccess_contents_bans_iplist!='') {
			...
			$htaccess_contents_bans_end = "RewriteRule !^(banned.php|youarebanned.jpg|ban.css)$ " . KU_BOARDSFOLDER . "banned.php [L]\n</IfModule>";
		} else {
