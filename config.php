...
if (!$cache_loaded) {
	/* Database */
		...
		$cf['KU_DBHOST']          = ''; /* Database hostname */
		$cf['KU_DBDATABASE']      = ''; /* Database... database */
		$cf['KU_DBUSERNAME']      = ''; /* Database username */
	...
	/* Root dir */
		$cf['KU_ROOTDIR']   = '';  /* Full system path of the folder containing kusaba.php, with trailing slash. If you're running a Windows server, add .'\\'; to the end of the folder name*/
		
	/* Chan info */
		$cf['KU_NAME']      = 'Hydrogen Beta 9'; /* The name of your site */
		$cf['KU_SLOGAN']    = '<em>"WHOA"</em>'; /* Site slogan, set to nothing to disable its display */
		...
		/*Hydrogen add-ons*/
		/*The ad that will be displayed directly below the postbox*/
		$cf['HY_HEADERAD'] = file_get_contents($cf['KU_ROOTDIR'] . 'hydrogen/ad_top.html');
		/*The ad that will be displayed at the bottom of the page*/
		$cf['HY_FOOTERAD'] = file_get_contents($cf['KU_ROOTDIR'] . 'hydrogen/ad_base.html');
		/*CYA LOL*/
		$cf['HY_DISCLAIMER'] = 'All trademarks and copyrights on this page are owned by their respective parties. Images uploaded are the responsibility of the Poster. Comments are owned by the Poster.';
		/*Analytics. The first will run whenever a board is accessed. The second will run whenever the menu is accessed. This is because the overchan (http://shii.org/2ch/) only links to boards and not the menu, meaning menu hits will be from people who access your site directly and not through the overchan. Leave them blank if you don't have anything set up*/
		
		$cf['HY_BOARDCOUNT'] = file_get_contents($cf['KU_ROOTDIR'] . 'hydrogen/analytics_board.html');
		$cf['HY_MENUCOUNT'] = file_get_contents($cf['KU_ROOTDIR'] . 'hydrogen/analytics_menu.html');
	/*Image to be displayed on the top of the menu.*/
		$cf['HY_MENUIMG'] = '';
	/*Word that will bypass the wordfilters (PENIS, etc.) MUST NOT CONTAIN ANYTHING OTHER THAN CHARACTERS OR NUMBERS!*/
		$cf['HY_WFBYPASS'] = '1077';
	/*This is the ban word. The ban word should be a random assortment of characters or numbers (no symbols or punctuation). If you want Hydrogen to autoban when someone posts a word (say, boxxy) you would set a wordfilter in the management menu to change boxxy to "boxxy <ban word>" (take out the <>)*/
		$cf['HY_BANWORD'] = 'rglnca';
	/*Amount of seconds someone is banned for saying a banned word*/
		$cf['HY_BANSECONDS'] = '10';
	/* Paths and URLs */
	...
	/* CSS styles */
		$cf['KU_STYLES']        = 'burichan:kusabax:futaba:photon:oxygen'; /* Styles which are available to be used for the boards, separated by colons, in lower case.  These will be displayed next to [Home] [Manage] if KU_STYLESWIKUHER is set to true */
		$cf['KU_DEFAULTSTYLE']  = 'oxygen'; /* If Default is selected in the style list in board options, it will use this style.  Should be lower case */
		...
		$cf['KU_MENUSTYLES']        = 'oxygen:kusabax'; /* Menu styles */
		$cf['KU_DEFAULTMENUSTYLE']  = 'oxygen'; /* Default menu style */
	...
	/* Post display */
		...
		$cf['KU_BANMSG']          = '<br><font color="#FF0000"><b>' . file_get_contents($cf['KU_ROOTDIR'] . 'hydrogen/postbanmsg.html') . '</b></font>'; /* The text to add at the end of a post if a ban is placed and "Add ban message" is checked */
	...
	/* Special Tripcodes */
		$cf['KU_TRIPS'] = array('#faggot'  => 'lol fag',
		                        '#dentalplan' => 'im great'); /* Special tripcodes which can have a predefined output.  Do not include the initial ! in the output.  Maximum length for the output is 30 characters.  Set to array(); to disable */
	...
	/* Misc config */
		...
		$cf['KU_RANDOMSEED']        = 'ufhewqfy89348yfwq7gz5r23589fgtxbvuyfbkstdfvdferwf'; /* Type a bunch of random letters/numbers here, any large amount (35+ characters) will do */
	...
	/* Language / timezone / encoding */
		...
		putenv('TZ=US/Eastern'); /* The time zone which the server resides in */
