...
function toggleMod() {
	...
	if ((bSaf) || (bOpera) || (bMoz))
		modbox_html = modbox.innerHTML;
	else modbox_html = modbox.text;
	
	modbox_html = modbox_html.toLowerCase();
	var newhtml = '<td></td><td></td>';
	
	if (modbox_html == newhtml) {
		var newhtml = '<br clear="right"/><br/><div align="right">[ <input name="deleteall" onclick="return confirm(\'Are you sure you want to delete this post/thread?\');" type="submit" value="' . _gettext('D') . '"/> &amp; <input name="banquickuser" type="submit" value="' . _gettext('B') . '"/> ]</div>';
	}
	
	if ((bSaf) || (bOpera) || (bMoz))
		modbox.innerHTML = newhtml;
	else modbox.text = newhtml;
